...
def handle_messages(message):
    ...
    # اگر کاربر اشتراک ویژه دارد (پاسخ با فونت مارک‌داون)
    if is_premium:
    ...
    # ۳ پیام اول کاملاً رایگان بدون نیاز به عضویت (پاسخ با فونت مارک‌داون)
    if msg_count < 3:
    ...
    # ۱۰ پیام بعد از عضویت کانال (پاسخ با فونت مارک‌داون)
    if msg_count < 13:
        increment_msg(user_id)
        bot.send_chat_action(message.chat.id, 'typing')
        ai_response = call_gemini_ai_direct(message.text)

...
<body>
    ...
    <script>
        // دریافت داده‌ها از بک‌اند جنگو/فلسک پایتون
...
@app.route('/')
def home():
    # خواندن اطلاعات از دیتابیس برای نمایش در سایت ادمین
    conn = sqlite3.connect('bot_database.db', timeout=20)
    cursor = conn.cursor()
    ...
    # آمار رشد روزانه برای نمودار
    cursor.execute("SELECT join_date, COUNT(*) FROM users GROUP BY join_date ORDER BY join_date DESC LIMIT 5")
    rows = cursor.fetchall()
    conn.close()
    
    # آماده‌سازی دیتای نمودار (روزهای اخیر باید از قدیم به جدید باشند)
    rows.reverse()
    dates = [row[0] for row in rows]
    counts = [row[1] for row in rows]
    
    # اگر اطلاعاتی ثبت نشده بود یک دیتای نمایشی بگذارد تا نمودار خالی نباشد
    if not dates:
        dates = ["امروز"]
        counts = [total_users]
...
# پرامپت جادویی جدید برای میخکوب کردن مخاطب و حل مشکل قاطی کردن زبان‌ها
SYSTEM_PROMPT = """
...
۳. فقط و فقط به زبان فارسی روان، بدون غلط املایی و نگارشی بنویسید. هرگز از کلمات خارجی (چینی، روسی، آلمانی و...) استفاده نکنید.
...
۶. در پاسخ‌های خود حتماً از فرمت مارک‌داون استفاده کنید; مثلاً کلمات کلیدی، تیترها و بخش‌های مهم را با فونت ضخیم (مانند **متن مهم**) بنویسید تا پیام‌ها فوق‌العاده شیک و خوانا شوند.
"""

# ==================== بخش دیتابیس ایمن و هماهنگ ====================
def init_db():
    conn = sqlite3.connect('bot_database.db', timeout=20)
    cursor = conn.cursor()
    cursor.execute('''
...
# ==================== ارتباط با هوش مصنوعی جدید و قوی ====================
def call_gemini_ai_direct(user_message):
    ...
    try:
        ...
        payload = {
            "model": "llama-3.1-8b-instant", # مدل فوق‌العاده سریع و پایدار برای فارسی
            "messages": [
